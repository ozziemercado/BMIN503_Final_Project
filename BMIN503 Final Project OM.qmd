...
initial_admissions$RACE <- fct_infreq(initial_admissions$RACE)
...
```{r}
# Identify patient who accessed their MyChart DURING their initial NICU admission
mychart_access <- filtered_message_data |>
...
initial_admissions <- initial_admissions |>
  mutate(
    MyChart_Access = ifelse(MRN %in% mychart_access$MRN, 1, 0)
  )
patients_accessed_mychart <- sum(initial_admissions$MyChart_Access == 1, na.rm = TRUE)
...
The initial part of this section contains descriptive statistics for patients admitted to the NICU from January 2018 to September 2024. Displayed below is a summary table for the overall patient data set split by access to MyChart.

```{r}
# Split initial_admissions into two groups
accessed_mychart <- initial_admissions %>% filter(MyChart_Access == 1)
not_accessed_mychart <- initial_admissions %>% filter(MyChart_Access == 0)

# Create summary tables for each group
summary_accessed <- accessed_mychart |>
  tbl_summary(
  ...
  bold_labels() %>%
  modify_caption("**Summary Table: Accessed MyChart**")

summary_not_accessed <- not_accessed_mychart |>
  tbl_summary(
    include = c(SEX, RACE, ETHNICITY, PREFERRED_LANGUAGE, PAYOR_GROUP,
                GESTATIONAL_AGE_TOTAL_DAYS, NICU_LOS_DAYS, INBORN_IND),
    statistic = list(
...
patient_data$EPISODE_START_DATE <- as.Date(patient_data$EPISODE_START_DATE)
...
```{r}
# Aggregate monthly totals directly from initial_admissions
monthly_data_initial <- initial_admissions |>
  mutate(month = as.Date(cut(EPISODE_START_DATE, breaks = "month"))) |>
  group_by(month) |>
  summarise(
    total_admissions = n(),  # Total admissions
    mychart_access = sum(MyChart_Access == 1, na.rm = TRUE)  # Patients who accessed MyChart
  )

# Create the combined plot
ggplot(monthly_data_initial, aes(x = month)) +
  geom_col(aes(y = total_admissions), fill = "skyblue", color = "black") +  # Bar chart for total admissions
  geom_line(aes(y = mychart_access), color = "red", size = 1.2) +  # Line chart for MyChart access
  labs(
    title = "Monthly NICU Admissions with MyChart Access",
    x = "Month",
    y = "Number of Patients",
    caption = "Bar chart: Total admissions; Red line: MyChart access"
  ) +
  theme(
